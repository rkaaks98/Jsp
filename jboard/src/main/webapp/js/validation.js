...
document.addEventListener('DOMContentLoaded', function(){
	...
	//유효성 검사에 사용할 상태변수 선언(let으로 선언해야함)
	let isUidOk = false;
	let isPassOk = false;
	let isNameOk = false;
	let isNickOk = false;
	...
	btnCheckUid.onclick = function(){
		...
		//아이디 유효성 검사
		if (!value.match(reUid)) {//value값에 매치가 되지 않으면
	...
	formRegister.pass2.addEventListener('focusout', function(){
		...
		if(!value1.match(rePass)){
			passResult.innerText = '비밀번호 숫자, 소문자, 대문자, 특수문자 조합 8자리.';
